...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $controller->deleteRegistro($_GET['id']);
}
...
// $result=$controller->listDias($user, $sortfield, $sortorder);
...
print '<td class="liste_titre" colspan="1">';
print '<input type="text" name="description" value="' . $description . '" />';
print '</td>';
...
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;  // Obtener la página actual (por defecto es 1)

$totalRecords = $controller->getRegistros($user, $filters);  // Obtener el número total de registros
	...
	foreach ($result as $obj) {
		...
		switch ($obj->status) {
			case 0:
				...
				$statusClass = "badge badge-status0 badge-status"; //"Borrador"
				break;
			case 1:
				$statusLabel = "Aprobada";
				$statusClass = "badge  badge-status4 badge-status"; //"Validado"
				break;
			case 9:
				$statusLabel = "Rechazada";
				$statusClass = "badge badge-status8 badge-status"; //"Cancelado"
				break;
			default:
		...
		print '<a class="fas  fa-eye" style="" title="Editar Registro" href="dias_permiso_card.php?action=edit&id=' . $obj->peticion_id  . '"></a>';
        print '<a class="fas  fa-trash" style="" title="Eliminar" href="' . $_SERVER["PHP_SELF"] . '?action=delete&id=' . $obj->peticion_id  . '"></a>';
		print '</td>';
